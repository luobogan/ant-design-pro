...
let DocsAdaptor = class DocsAdaptor extends ObjectAdaptor {
	...
	convert(pageElement, mainScene) {
		...
		const documents = new Documents("__DocsRender__", documentSkeleton);
		...
		const scene = new Scene("__DocsScene__" + id, sv);
		const viewMain = new Viewport("__DocsViewPort_" + id, scene, {
			left: 0,
			top: 0,
			bottom: 0,
			right: 0,
...
let SlideAdaptor = class SlideAdaptor extends ObjectAdaptor {
	...
	convert(pageElement, mainScene) {
		...
		const slideComponent = new Slide("__SLIDERender__" + id, {
			top,
			left,
			width,
			height,
...
let SpreadsheetAdaptor = class SpreadsheetAdaptor extends ObjectAdaptor {
	...
	convert(pageElement, mainScene) {
		...
		const sv = new SceneViewer("spreadInSlideSceneViewer" + id, {
			top,
			left,
			width,
			height,
		...
		const scene = new Scene("spreadInSlideScene" + id, sv, {
			width: allWidth,
			height: allHeight
		});
		...
		const spreadsheetRowHeader = new SpreadsheetRowHeader("spreadInSlideRow", spreadsheetSkeleton);
		const spreadsheetColumnHeader = new SpreadsheetColumnHeader("spreadInSlideColumn", spreadsheetSkeleton);
		const SpreadsheetLeftTopPlaceholder = new Rect("spreadInSlideLeftTop", {
			zIndex: 2,
			left: -1,
			top: -1,
	...
	_updateViewport(id, rowHeaderWidth, columnHeaderHeight, scene, mainScene) {
		...
		const viewMain = new Viewport("spreadInSlideViewMain" + id, scene, {
		...
		const viewTop = new Viewport("spreadInSlideViewTop" + id, scene, {
			left: rowHeaderWidthScale,
		...
		const viewLeft = new Viewport("spreadInSlideViewLeft" + id, scene, {
			left: 0,
			bottom: 0,
			top: columnHeaderHeightScale,
